...
def one_random_experiment(architecture, exps=50, num: int = 1000, one=True, return_sth=False, pre_path='', normal_dist=False, 
  ...
  this_path = file_name_handling('random_data_random_untrained_network', architecture, num=str(num), exps=exps, pre_path=pre_path, 
  ...
  plotting_actions({res1, stable_ranks_all, simple_spherical_mean_width_all, spherical_mean_width_v2_all, eigen_count}, num, this_path, net, dis_stats, display_neuron_matrx_s, cell_dims) # type: ignore
  ...
  plot_gifs(layer_activation_ratio, eigens, dist_all, list_pca_2d, list_pca_3d, list_random_2d, list_random_3d, this_path, costume_range=max(np.abs(scale * 2), 10, int(np.abs(loc / 2))), pre_path=this_path, scale=scale, eigenvectors=np.array(eigen_vectors, dtype=object)) # type: ignore
...
def before_after_training_experiment(architecture, num=1000, epochs=50, pre_path='', normal_dist=False, loc=0, scale=1):
  ...
  this_path = file_name_handling('random_data_random_trained_network', architecture, num, pre_path=pre_path, normal_dist=normal_dist, loc=loc, scale=scale) # type: ignore
  ...
  stable_neuron_analysis(simple_model, train[0]) # type: ignore
  eigen_count, eigens = simple_model.other_forward_analysis(train[0], True)

  plotting_actions(simple_model.additive_activations, eigen_count, train[0].shape[0], this_path, simple_model, suffix='training_data_before_training_') # type: ignore
  ...
  animate_histogram(layer_activation_ratio, 'layer ', save_path='layer_wise_pretraining_training_data.gif', pre_path=this_path) # type: ignore
  animate_histogram(eigens, 'layers: ', x_axis_title='eigenvalues distribution', save_path='eigenvalues_layer_wise.gif', pre_path=this_path) # type: ignore

  simple_model.reset_all()

  va = train_model(simple_model, train_loader, val_loader, epochs=epochs, validation_data=torch.tensor(val[0])) # type: ignore
  simple_model.to('cpu')
  animate_histogram(va, 'epoch ', save_path='epoch_visualization.gif', pre_path=this_path) # type: ignore
  stable_neuron_analysis(simple_model, train[0]) # type: ignore
  eigen_count, eigens = simple_model.other_forward_analysis(train[0], True)

  plotting_actions(simple_model.additive_activations, eigen_count, train[0].shape[0], this_path, simple_model, suffix='training_data_after_training_') # type: ignore

  layer_activation_ratio = simple_model.analysis_neurons_layer_wise_animation(simple_model.additive_activations, train[0].shape[0])
  animate_histogram(layer_activation_ratio, 'layer ', save_path='layer_wise_after_training.gif', pre_path=this_path) # type: ignore
  animate_histogram(eigens, 'layers: ', x_axis_title='eigenvalues distribution', save_path='eigenvalues_layer_wise.gif', pre_path=this_path) # type: ignore
...
def mnist_training_analysis(architecture, epochs=50, pre_path=''):
  ...
  train_loader, val_loader, test_loader = get_mnist_data_loaders() # type: ignore

  train_x = train_loader.dataset.dataset.data[train_loader.dataset.indices,:,:].to(torch.float32).to(device).flatten(1) # type: ignore
  val_x = val_loader.dataset.dataset.data[val_loader.dataset.indices,:,:].to(torch.float32).to(device).flatten(1) # type: ignore
  test_x = test_loader.dataset.data.to(torch.float32).to(device).flatten(1) # type: ignore
  ...
  train_add, train_eig, val_add, val_eig = train_model(simple_model, train_loader, test_loader, base_path=this_path, train_x=train_x, val_x=val_x, val_loader=val_loader, epochs=epochs, loss='crossentropy') # type: ignore
  ...
  # simple_model.to('cpu')
  animate_histogram(train_add, 'epoch ', save_path='epoch_visualization_train.gif', pre_path=this_path) # type: ignore
  animate_histogram(val_add, 'epoch ', save_path='epoch_visualization_val.gif', pre_path=this_path) # type: ignore

  animate_histogram(train_eig, 'layers: ', x_axis_title='eigenvalues distribution', save_path='eigenvalues_layer_wise_train.gif', pre_path=this_path, fixed_scale=True, custom_range=1) # type: ignore
  animate_histogram(val_eig, 'layers: ', x_axis_title='eigenvalues distribution', save_path='eigenvalues_layer_wise_val.gif', pre_path=this_path, fixed_scale=True, custom_range=1) # type: ignore
