...
def one_random_dataset_run(model, n, d):
  '''
    Parameters
    model (torch.nn.Module): A trained PyTorch neural network model.
  ...
  x, y = create_random_data(d, n)
  stable_neuron_analysis(model, x)

  return model.additive_activations, model.additive_activation_ratio


def one_random_experiment(architecture, exps=500, num=1000, one=True, return_sth=False):
  '''
    Parameters
  ...
  '''
  this_path = 'random_data_random_untrained_network{}_exps{}_num{}/'.format(str(architecture), str(exps), str(num))
  ...
  for i in range(exps):
    r1, r2 = one_random_dataset_run(net, 1000, architecture[0])
    res_run1 += [r1]
    res_run2 += [r2]
    net.reset()
